...
public class Serial
{
     ...
     * -1: Ung�ltiger Portname
     * -2: Ung�ltige Baudrate
     * -3: Ung�ltige Databists
     * -4: Ung�ltige Stopbits
     * -5: Ung�litge Parit�t
     * -6: Weiterer, nicht genauer spezifizierter Fehler 
     * 
     * @param portName Der Port der geoeffnet werden soll. Zum Beispiel COM11.
     * @param baudrate Die �bertragungsrate die verwendet werden soll. Beispiel: 9600
    ...
    public Serial(String portName, int baudrate, int dataBits, int stopBits, int parity)// throws Exception
    {
        ...
        {
            if(DEBUG_SERIAL)
                System.out.println("Ung�ltiger Wert in portName [" + portName +"]");
        ...
        {
            if(DEBUG_SERIAL)
                System.out.println("Ung�ltiger Wert in baudrate [" + baudrate +"]");
        ...
        {
            if(DEBUG_SERIAL)
                System.out.println("Ung�ltiger Wert in dataBits [" + dataBits +"]");
        ...
        {
            if(DEBUG_SERIAL)
                System.out.println("Ung�ltiger Wert in stopBits [" + stopBits +"]");
        ...
        {
            if(DEBUG_SERIAL)
                System.out.println("Ung�ltiger Wert in parity [" + parity +"]");
    ...
    public boolean open()
    {
        ...
        else
        {
            this.isOpen = false;
            if(DEBUG_SERIAL)
                System.out.println("Die Schnittstelle konnte nicht ge�ffnet werden");
            return false;
        }
    }
    ...
    public int dataAvailable()
    {
        ...
        try
        {
            ...
            if(DEBUG_SERIAL)
                System.out.println("... und liefert den Wert " + av + " zur�ck");
            return av;
        }
        catch (Exception e)
        {
    ...
    public int read()
    {
        ...
        try
        {
            ...
            int rb = readByte();
            if(DEBUG_SERIAL)
                System.out.println("... und liefert den Wert " + rb + " zur�ck");
            return rb;
        }
        catch(Exception ioe)
        {
    ...
    /**
     * liest bis zu len Bytes von der seriellen Schnittstelle in das �bergebene Byte-Array<br>
     * die Anzahl der tats�chlich gelesenen Bytes wird zur�ckgeliefert, bzw. liefert -1 zur�ck,<br>
     * wenn die Schnittstelle nicht ge�ffnet ist oder nicht gelesen werden konnte.
     * <b><font color=red>Zu pruefen, ob das korrekt ist <br>
     * Die Methode blockiert, bis mindestens ein Byte(wert) verfuegbar ist. ??? </font></b>
     * @param b das �bergebene byte-Array. Dieses muss bereits deklariert und definiert worden sein (mit Mindestl�nge 1)
     * @param len maximale Anzahl der Bytes, die gelesen werden sollen. Sind weniger Bytes verf�gbar, werden nur diese in das Array �bertragen 
     * @return tats�chliche Anzahl der in das byte-Array �bertragene Werte oder -1, wenn nicht auf die Schnittstelle zugegriffen werden konnte. 
     */
    ...
    /**
     * Diese Methode sendet das �bergebene Byte Array �ber die ge�ffnete serielle Schnittstelle.
     * Wenn nicht gesendet werden konnte, passiert nichts
     */
